...
- Public Driver self-registration with server-controlled role assignment
- JWT access tokens
...
The endpoint is public so a new Driver can create an account before logging in. The request intentionally has no `role` field. The backend always assigns exactly `DRIVER`, which prevents users from self-registering as Admin, Fleet Manager, Safety Officer, or Financial Analyst.

The response is a `UserSummary` with `enabled: true` and `roles: ["DRIVER"]`. The Driver then signs in through `/api/auth/login` and can access the dashboard, vehicle/driver read views, and trip operations allowed by the RBAC matrix.
...
Test the new public Driver registration flow and its RBAC restrictions:

```powershell
.\scripts\signup-driver-smoke-test.ps1
```

This test signs up a unique Driver account, logs in, opens the dashboard and trips API, verifies that `/api/admin/users` returns `403`, and logs out.
